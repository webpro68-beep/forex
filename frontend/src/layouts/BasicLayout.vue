    ...
    >
      ...
      <template #menuHeaderRender>
        <div class="sidebar-logo-wrapper" :class="{ 'sidebar-logo-wrapper--collapsed': collapsed }">
          ...
          <div v-else class="sidebar-logo-content">
            <img :src="currentLogo" class="sidebar-logo" :alt="brandConfig.app_name" />
            <div class="sidebar-logo-details">
              <div class="sidebar-brand-name">{{ brandConfig.app_name || title }}</div>
              <div class="sidebar-brand-tagline">{{ brandConfig.tagline || 'AI-driven trading intelligence' }}</div>
            </div>
          </div>
        </div>
      </template>
    ...
    -->
      <template #headerContentRender>
        <div class="custom-header-left">
          <div class="custom-header-title">{{ currentPageTitle }}</div>
          <div class="custom-header-subtitle">{{ headerSubtitle }}</div>
        </div>
        <div class="custom-header-metrics">
          <div class="custom-header-metric-item">
            <div class="metric-label">SESSION</div>
            <div class="metric-value">{{ sessionCount }}</div>
          </div>
          <div class="custom-header-metric-item">
            <div class="metric-label">BALANCE</div>
            <div class="metric-value">{{ balanceDisplay }}</div>
          </div>
          <div class="custom-header-metric-item">
            <div class="metric-label">PNL</div>
            <div :class="['metric-value', pnlSignClass]">{{ pnlDisplay }}</div>
          </div>
        </div>
        <div class="custom-header-actions">
          <div class="custom-header-chip">
...
export default {
  ...
  computed: {
    ...
    currentPageTitle () {
      const title = (this.$route && this.$route.meta && this.$route.meta.title) ? this.$route.meta.title : this.title
      return i18nRender(title)
    },
    headerSubtitle () {
      return (this.brandConfig && this.brandConfig.tagline) || 'AI-powered trading intelligence'
    },
    marketStatus () {
      return (this.brandConfig && this.brandConfig.market_status) || 'LIVE'
    },
    sessionCount () {
      return (this.brandConfig && this.brandConfig.trading_session_count) || 12
    },
    balanceDisplay () {
      return (this.brandConfig && this.brandConfig.account_balance) || '$82,389'
    },
    pnlDisplay () {
      return (this.brandConfig && this.brandConfig.pnl_change) || '+3.4%'
    },
    pnlSignClass () {
      const value = this.pnlDisplay || ''
      return value.startsWith('-') ? 'metric-negative' : 'metric-positive'
    }
  },
  created () {
