...
class BacktestOptimizationAgent:
    ...
    def run(self, symbol: str, timeframe: str, step_range: list[float], x_range: list[int], risk_range: list[float]) -> WinnerGene:
        ...
        for step in step_range:
            for x in x_range:
                for risk in risk_range:
                    ...
                    gene = WinnerGene(symbol=symbol, timeframe=timeframe, step_size=step, x_level=x, risk_pct=risk, atr_period=14, profit_factor=profit_factor, max_drawdown=max_dd, recovery_factor=recovery, win_rate=round(random.uniform(0.42, 0.68), 3), expectancy=round(random.uniform(-0.2, 0.8), 3))
